...
export function SkillListEditor({ label, skills, otherSkills, onChange, error, max = 20, inputRef, inputId, errorId }: SkillListEditorProps) {
  ...
  return <div>
    <label className="text-sm font-semibold text-depth" htmlFor={inputId ?? `skill-input-${label.replaceAll(' ', '-').toLowerCase()}`}>{label}</label>
    <div className="mt-2 flex gap-2">
      <input ref={inputRef} id={inputId ?? `skill-input-${label.replaceAll(' ', '-').toLowerCase()}`} className="h-10 min-w-0 flex-1 rounded-aura-sm border border-harbor/20 bg-white px-3 text-sm text-depth focus:border-marine focus:outline-none focus:ring-2 focus:ring-glacier/35" value={input} onChange={(event) => { setInput(event.target.value); setInputError('') }} onKeyDown={handleKeyDown} placeholder="Type a skill and press Enter" aria-invalid={Boolean(error) || undefined} aria-describedby={error ? errorId : undefined} />
      <Button variant="secondary" onClick={addSkill}><Plus size={16} />Add</Button>
    </div>
    {inputError || error ? <p id={error ? errorId : undefined} className="mb-0 mt-1.5 text-xs text-aura-danger" role="alert">{inputError || error}</p> : null}
