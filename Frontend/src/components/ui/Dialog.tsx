...
export function Dialog({ open, title, children, onClose }: DialogProps) {
  ...
  return (
    ...
    >
      <section
        className="dialog flex max-h-[88vh] w-[min(92vw,42rem)] flex-col overflow-hidden rounded-aura-lg border border-harbor/20 bg-white shadow-aura-lg"
        role="dialog"
        aria-modal="true"
      ...
      >
        <header className="dialog__header flex items-center justify-between gap-5 border-b border-harbor/15 bg-white px-5 py-4">
          <h2 id="dialog-title" className="m-0 text-lg font-semibold text-depth">
            {title}
          </h2>
          <button
            type="button"
