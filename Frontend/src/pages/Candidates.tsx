...
import { selectCandidateListItems, selectJobById } from '../store/demoSelectors'
...
export default function Candidates() {
  ...
  const filteredItems = allItems.filter((item) => {
    ...
    const itemRecommendation = item.screeningEvaluation?.recommendation
    ...
    return matchesSearch && matchesRecommendation && matchesStage && matchesJob
  })
  ...
  return (
    ...
    >
      <Card className="mb-4 p-4">
        <div className={`grid gap-3 ${job ? 'md:grid-cols-[minmax(240px,1fr)_220px_220px]' : 'md:grid-cols-[minmax(240px,1fr)_210px_190px_210px]'}`}>
      ...
      {filteredItems.length > 0 ? (
        <><div className="hidden lg:block"><CandidateTable items={filteredItems} /></div><div className="grid gap-3 lg:hidden">{filteredItems.map((item) => <CandidateCard item={item} key={item.application.id} />)}</div></>
      ) : (
