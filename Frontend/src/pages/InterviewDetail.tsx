...
import { selectInterviewById } from '../store/demoSelectors'
  ...
  return <PageContainer eyebrow="Interview operations" title={`${candidate.fullName} interview`} description={`${job.title} · ${formatInterviewStatus(interview.status)}`} actions={<Link className={linkClass} to="/interviews">Back to interviews</Link>}>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
      <div className="grid gap-4">
        <Card className="p-5 md:p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="m-0 text-[10px] font-bold uppercase tracking-[0.14em] text-marine">Interview overview</p><h2 className="mb-0 mt-2 text-xl font-semibold text-depth">{candidate.fullName}</h2><p className="mb-0 mt-1 text-sm text-aura-text-secondary">{candidate.currentPosition} · {job.title}</p></div><Badge tone={interview.status === 'COMPLETED' ? 'success' : interview.status === 'CANCELLED' ? 'neutral' : 'accent'}>{formatInterviewStatus(interview.status)}</Badge></div></Card>
