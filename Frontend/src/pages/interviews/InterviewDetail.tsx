...
function DemoInterviewDetail({ interviewId }: { interviewId: string }) {
  ...
  const reviewStatus = interview ? selectInterviewAnalysisPreparationStatus(state, interview.id) : 'TRANSCRIPT_REQUIRED'
  const finalEvaluation = candidate && application ? selectLatestFinalEvaluation(state, candidate.id, application.id) : undefined
  ...
  return <PageContainer eyebrow="Interview operations" title={`${candidate.fullName} interview`} description={`${job.title} · ${formatInterviewStatus(interview.status)}`} actions={<div className="flex flex-wrap gap-2"><Link className={linkClass} to="/interviews">Back to interviews</Link>{preparationStatus === 'APPROVED' && interview.status !== 'CANCELLED' ? <Link className={linkClass} to={`/interviews/${interview.id}/session`}>{sessionStatus === 'READY' ? 'Open session workspace' : sessionStatus === 'IN_PROGRESS' ? 'Return to session' : sessionStatus === 'PAUSED' ? 'Resume session' : 'View session summary'}</Link> : null}</div>}>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
      ...
      <div className="grid content-start gap-4"><Card className="p-5 md:p-6"><h2 className="m-0 text-lg font-semibold text-depth">Candidate context</h2><dl className="mt-4 grid gap-3 text-sm"><div><dt className="text-xs text-aura-text-muted">Email</dt><dd className="mb-0 mt-1 font-medium text-depth">{candidate.email}</dd></div><div><dt className="text-xs text-aura-text-muted">Experience</dt><dd className="mb-0 mt-1 font-medium text-depth">{candidate.yearsExperience} years</dd></div></dl><Link className="mt-4 inline-flex font-semibold text-harbor" to={`/candidates/${candidate.id}`}>Open candidate profile</Link></Card><Card className="p-5 md:p-6"><h2 className="m-0 text-lg font-semibold text-depth">Interview preparation</h2><Badge tone={preparationStatus === 'APPROVED' ? 'success' : preparationStatus === 'FAILED' ? 'warning' : 'accent'}>{preparationStatus === 'APPROVED' ? 'Plan approved' : preparationStatus === 'DRAFT_READY' ? 'Draft ready' : preparationStatus === 'FAILED' ? 'Preparation failed' : 'Preparing'}</Badge><p className="mb-0 mt-3 text-sm leading-6 text-aura-text-secondary">{preparationStatus === 'APPROVED' ? `Interview plan approved · ${questionSet?.questions.length ?? 0} approved questions.` : preparationStatus === 'DRAFT_READY' ? `Interview questions are ready for review · ${questionSet?.questions.length ?? 0} questions.` : preparationStatus === 'FAILED' ? 'Question preparation requires attention.' : 'AURA is preparing candidate-specific interview questions.'}</p><Link className={`${linkClass} mt-4`} to={`/interviews/${interview.id}/questions`}>{preparationStatus === 'APPROVED' ? 'View question plan' : preparationStatus === 'FAILED' ? 'Review preparation issue' : 'Review questions'}</Link></Card>{interview.status === 'SCHEDULED' ? <Card className="p-5"><p className="m-0 text-sm leading-6 text-aura-text-secondary">The candidate can reschedule from their scheduling link within the policy limit.</p><Button className="mt-4" variant="ghost" onClick={() => setCancelOpen(true)}>Cancel interview</Button></Card> : interview.status === 'CANCELLED' ? <Card className="p-5"><p className="m-0 text-sm text-aura-text-secondary">The record remains available for audit. The application stage is preserved.</p></Card> : null}</div>
      {interview.status === 'COMPLETED' ? <Card className="p-5 md:p-6 xl:col-span-2"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="m-0 text-lg font-semibold text-depth">Post-interview review</h2><p className="mb-0 mt-2 text-sm text-aura-text-secondary">{reviewStatus === 'APPROVED' ? 'Reviewed interview evidence is ready for the next evaluation phase.' : reviewStatus === 'DRAFT_READY' ? 'AURA prepared evidence-backed analysis for interviewer review.' : reviewStatus === 'PREPARING' ? 'AURA is extracting evidence from the approved transcript.' : 'Complete the transcript review before the analysis can be finalized.'}</p></div><Badge tone={reviewStatus === 'APPROVED' ? 'success' : reviewStatus === 'FAILED' ? 'warning' : 'accent'}>{reviewStatus === 'TRANSCRIPT_REQUIRED' ? 'Transcript needed' : reviewStatus === 'TRANSCRIPT_DRAFT' ? 'Transcript in review' : reviewStatus === 'PREPARING' ? 'Analysis preparing' : reviewStatus === 'DRAFT_READY' ? 'Analysis ready' : reviewStatus === 'APPROVED' ? 'Analysis approved' : 'Preparation failed'}</Badge></div><div className="mt-4 flex flex-wrap gap-2"><Link className={linkClass} to={reviewStatus === 'TRANSCRIPT_REQUIRED' || reviewStatus === 'TRANSCRIPT_DRAFT' ? `/interviews/${interview.id}/transcript` : `/interviews/${interview.id}/analysis`}>{reviewStatus === 'TRANSCRIPT_REQUIRED' ? 'Add transcript' : reviewStatus === 'TRANSCRIPT_DRAFT' ? 'Review transcript' : reviewStatus === 'DRAFT_READY' ? 'Review analysis' : reviewStatus === 'APPROVED' ? 'View analysis' : 'View preparation'}</Link>{reviewStatus === 'TRANSCRIPT_REQUIRED' ? <DemoEvaluationFastForwardAction interviewId={interview.id} /> : null}</div></Card> : null}
      {reviewStatus === 'APPROVED' ? <Card className="p-5 md:p-6 xl:col-span-2"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="m-0 text-lg font-semibold text-depth">Final evaluation</h2><p className="mb-0 mt-2 text-sm text-aura-text-secondary">{!finalEvaluation ? 'AURA is preparing the system evidence score.' : finalEvaluation.status === 'DECIDED' ? `Final decision recorded · ${finalEvaluation.humanDecision?.toLocaleLowerCase()} by ${finalEvaluation.decidedByRole?.replaceAll('_', ' ').toLocaleLowerCase()}.` : `System evaluation ready · ${finalEvaluation.weightedEvidenceScore === undefined ? 'Evidence score unavailable' : `Evidence score ${finalEvaluation.weightedEvidenceScore} / 100`} · ${finalEvaluation.systemRecommendation.replaceAll('_', ' ').toLocaleLowerCase()}.`}</p></div><Badge tone={finalEvaluation?.status === 'DECIDED' ? 'success' : 'accent'}>{finalEvaluation?.status === 'DECIDED' ? 'Final decision recorded' : finalEvaluation ? 'System evaluation ready' : 'Preparing'}</Badge></div>{finalEvaluation ? <Link className={`${linkClass} mt-4`} to={`/candidates/${candidate.id}/final-evaluation`}>Review final evaluation</Link> : null}</Card> : null}
    </div>
    <Dialog open={cancelOpen} title="Cancel interview" onClose={() => setCancelOpen(false)}><p className="mt-0 text-sm leading-6 text-aura-text-secondary">Cancel the scheduled interview for {candidate.fullName}?</p><p className="mb-0 mt-3 text-xs leading-5 text-aura-text-muted">No candidate communication will be sent automatically. The broad application stage will be preserved.</p><div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button variant="ghost" onClick={() => setCancelOpen(false)}>Keep interview</Button><Button onClick={cancelInterview}>Cancel interview</Button></div></Dialog>
  </PageContainer>
}
