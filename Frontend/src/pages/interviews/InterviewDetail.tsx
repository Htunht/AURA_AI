...
import { selectInterviewAnalysisPreparationStatus, selectInterviewById, selectInterviewQuestionPreparationStatus, selectLatestInterviewQuestionSet, selectInterviewSessionOperationalStatus } from '../../store/demoSelectors'
...
export default function InterviewDetail() {
  ...
  return <PageContainer eyebrow="Interview operations" title={`${candidate.fullName} interview`} description={`${job.title} · ${formatInterviewStatus(interview.status)}`} actions={<div className="flex flex-wrap gap-2"><Link className={linkClass} to="/interviews">Back to interviews</Link>{preparationStatus === 'APPROVED' && interview.status !== 'CANCELLED' ? <Link className={linkClass} to={`/interviews/${interview.id}/session`}>{sessionStatus === 'READY' ? 'Open session workspace' : sessionStatus === 'IN_PROGRESS' ? 'Return to session' : sessionStatus === 'PAUSED' ? 'Resume session' : 'View session summary'}</Link> : null}</div>}>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
      ...
      {interview.status === 'COMPLETED' ? <Card className="p-5 md:p-6 xl:col-span-2"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="m-0 text-lg font-semibold text-depth">Post-interview review</h2><p className="mb-0 mt-2 text-sm text-aura-text-secondary">{reviewStatus === 'APPROVED' ? 'Reviewed interview evidence is ready for the next evaluation phase.' : reviewStatus === 'DRAFT_READY' ? 'AURA prepared evidence-backed analysis for interviewer review.' : reviewStatus === 'PREPARING' ? 'AURA is extracting evidence from the approved transcript.' : 'Complete the transcript review before the analysis can be finalized.'}</p></div><Badge tone={reviewStatus === 'APPROVED' ? 'success' : reviewStatus === 'FAILED' ? 'warning' : 'accent'}>{reviewStatus === 'TRANSCRIPT_REQUIRED' ? 'Transcript needed' : reviewStatus === 'TRANSCRIPT_DRAFT' ? 'Transcript in review' : reviewStatus === 'PREPARING' ? 'Analysis preparing' : reviewStatus === 'DRAFT_READY' ? 'Analysis ready' : reviewStatus === 'APPROVED' ? 'Analysis approved' : 'Preparation failed'}</Badge></div><Link className={`${linkClass} mt-4`} to={reviewStatus === 'TRANSCRIPT_REQUIRED' || reviewStatus === 'TRANSCRIPT_DRAFT' ? `/interviews/${interview.id}/transcript` : `/interviews/${interview.id}/analysis`}>{reviewStatus === 'TRANSCRIPT_REQUIRED' ? 'Add transcript' : reviewStatus === 'TRANSCRIPT_DRAFT' ? 'Review transcript' : reviewStatus === 'DRAFT_READY' ? 'Review analysis' : reviewStatus === 'APPROVED' ? 'View analysis' : 'View preparation'}</Link></Card> : null}
    </div>
