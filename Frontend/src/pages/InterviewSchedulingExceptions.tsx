...
import { getSchedulingExceptionLabel, selectSchedulingAutomationViewModels } from '../store/demoSelectors'
              ...
              <Card className="p-5" key={model.invitation.id}>
                <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <Link className={linkClass} to={`/interviews/policies/${model.job.id}`}>Review scheduling setup</Link>
                    <Link className={linkClass} to={`/candidates/${model.candidate.id}`}>Open candidate</Link>
                    <Button variant="secondary" onClick={() => model.state === 'EXPIRED' ? regenerateInvitation(model.invitation.id) : retryAutomation(model.invitation.applicationId)}><RotateCcw size={15} />{model.state === 'EXPIRED' ? 'Prepare new invitation' : 'Retry automation'}</Button>
                  </div>
                </div>
              </Card>
            )
