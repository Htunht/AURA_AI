import { ArrowRight, BriefcaseBusiness, Pencil, Plus, Search } from 'lucide-react'
...
import { selectApplicationCountByJobId, selectJobReadiness, selectPublishedApplicationFormByJobId, selectPublishedRubricByJobId } from '../store/demoSelectors'
...
function readinessLabel(status: ReturnType<typeof selectJobReadiness>['status'], jobStatus: JobStatus) {
  if (jobStatus === 'CLOSED') return 'Closed'
  if (jobStatus === 'ARCHIVED') return 'Archived'
  return { JOB_DETAILS_INCOMPLETE: 'Complete job details', APPLICATION_FORM_REQUIRED: 'Application form required', SCREENING_SETUP_REQUIRED: 'Screening setup required', READY: 'Ready to open', OPEN: 'Open' }[status]
}
...
export default function Jobs() {
  ...
  return <PageContainer title="Job openings" description="Create roles, track setup readiness, and manage each hiring lifecycle." actions={<Link className={createClass} to="/jobs/new"><Plus size={16} />Create job</Link>}>
    ...
    {jobs.length ? <div className="grid gap-4 xl:grid-cols-2">{jobs.map((job) => {
      ...
      return <Card className="group flex flex-col gap-4 p-5 transition-colors hover:border-marine/35 md:p-6" key={job.id}><div className="flex items-start justify-between gap-4"><div><p className="m-0 text-[10px] font-bold uppercase tracking-[0.14em] text-marine">{job.department} · {typeLabels[job.employmentType]}</p><h2 className="mb-0 mt-2 text-lg font-semibold text-depth">{job.title}</h2></div><Badge tone={statusTone(job.status)}>{job.status}</Badge></div><p className="m-0 line-clamp-2 text-sm leading-6 text-aura-text-secondary">{job.description}</p><div className="rounded-aura-sm border border-harbor/10 bg-frost/55 p-3"><p className="m-0 text-[10px] font-bold uppercase tracking-wide text-aura-text-muted">Readiness</p><p className={`mb-0 mt-1 text-sm font-semibold ${readiness.ready ? 'text-aura-success' : 'text-aura-warning'}`}>{readinessLabel(readiness.status, job.status)}</p>{readiness.issues[0] ? <p className="mb-0 mt-1 text-xs text-aura-text-muted">{readiness.issues[0]}</p> : null}</div><dl className="grid grid-cols-2 gap-2 sm:grid-cols-4"><div><dt className="text-[10px] uppercase tracking-wide text-aura-text-muted">Positions</dt><dd className="mb-0 mt-1 text-sm font-semibold text-depth">{job.positionsCount}</dd></div><div><dt className="text-[10px] uppercase tracking-wide text-aura-text-muted">Deadline</dt><dd className="mb-0 mt-1 text-sm font-semibold text-depth">{formatDate(job.applicationDeadline)}</dd></div><div><dt className="text-[10px] uppercase tracking-wide text-aura-text-muted">Applications</dt><dd className="mb-0 mt-1 text-sm font-semibold text-depth">{count}</dd></div><div><dt className="text-[10px] uppercase tracking-wide text-aura-text-muted">Setup</dt><dd className="mb-0 mt-1 text-xs font-semibold text-depth">Form {form ? 'published' : 'required'}<br />Screening {rubric ? 'published' : 'required'}</dd></div></dl><div className="mt-auto flex flex-wrap items-center gap-4 border-t border-harbor/10 pt-4"><Link className="inline-flex items-center gap-1.5 text-sm font-semibold text-harbor no-underline hover:text-depth" to={`/jobs/${job.id}`}><ArrowRight size={16} />View</Link><Link className="inline-flex items-center gap-1.5 text-sm font-semibold text-harbor no-underline hover:text-depth" to={`/jobs/${job.id}/edit`}><Pencil size={15} />Edit</Link></div></Card>
