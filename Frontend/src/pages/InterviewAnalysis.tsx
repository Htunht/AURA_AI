...
export default function InterviewAnalysis() {
  ...
  if (!interview || !candidate || !job || !transcript || transcript.status !== 'APPROVED' || !questionSet) return <PageContainer eyebrow="Interview analysis" title="Transcript approval required"><Link className={linkClass} to={`/interviews/${interviewId}/transcript`}>Review transcript</Link></PageContainer>
  ...
  return <PageContainer eyebrow="Interview analysis" title="Review interview evidence" description="Review AURA’s evidence extraction, correct the analysis, and approve it for final candidate evaluation." actions={<Link className={linkClass} to={`/interviews/${interview.id}/transcript`}>View transcript</Link>}>
    ...
    <Card className="mb-4 p-4"><div className="flex flex-wrap items-center gap-3"><strong className="text-depth">{candidate.fullName}</strong><span className="text-sm text-aura-text-secondary">{job.title}</span><Badge tone={analysis.status === 'APPROVED' ? 'success' : 'accent'}>{analysis.status === 'APPROVED' && analysis.approvedBy === 'AURA Demo Automation' ? 'Demo-approved analysis' : analysis.status === 'APPROVED' ? 'Analysis approved' : 'Draft analysis'}</Badge><span className="text-xs text-aura-text-muted">{analysis.evidence.length} evidence items</span></div><p className="mb-0 mt-3 text-xs text-aura-text-muted">{analysis.approvedBy === 'AURA Demo Automation' ? 'This simulated analysis was validated and approved by demo automation. Review it before using the scoring workspace.' : 'AURA prepared this advisory draft from the approved transcript, interview notes, and question plan. Human review is required.'}</p></Card>
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]">
      <main className="grid gap-4">
        ...
        <div className="grid gap-3">{analysis.evidence.map((evidence, evidenceIndex) => {
          ...
          return <section className="rounded-aura-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-aura-danger/35" key={evidence.id} ref={(node) => { evidenceCards.current[evidence.id] = node }} tabIndex={-1}>
            <Card className={`p-5 ${issue ? 'border-aura-danger/45 bg-aura-danger-soft/20' : ''}`}>
              ...
              {issue ? <div className="mt-4 rounded-aura-sm border border-aura-danger/20 bg-white p-3" role="alert"><p className="m-0 text-xs font-semibold text-aura-danger">Evidence item {evidenceIndex + 1} has {issue.reasons.length} blocking {issue.reasons.length === 1 ? 'issue' : 'issues'}:</p>{issue.reasons.map((reason) => <p className="mb-0 mt-1 text-xs text-aura-danger" key={`${reason.field}-${reason.message}`}>{reason.message}</p>)}</div> : null}
        ...
        <div className="grid gap-4 md:grid-cols-3">{([['strengths', 'Strengths'], ['concerns', 'Concerns'], ['missingEvidence', 'Missing evidence']] as const).map(([key, label]) => {
          const duplicates = readiness.duplicateListItems.filter((item) => item.listKey === key)
          return <Card className={duplicates.length ? 'border-aura-danger/35 p-4' : 'p-4'} key={key}><label className="text-sm font-semibold text-depth">{label}<textarea aria-invalid={Boolean(duplicates.length)} className={`${fieldClass} min-h-48 ${duplicates.length ? invalidFieldClass : ''}`} disabled={!editable} value={analysis[key].join('\n')} onChange={(event) => updateList(key, event.target.value)} /></label><p className="mb-0 mt-2 text-xs text-aura-text-muted">One item per line.</p>{duplicates.map((duplicate) => <div className="mt-3 rounded-aura-sm bg-aura-danger-soft p-3" key={`${duplicate.listKey}-${duplicate.duplicateIndex}`}><p className="m-0 text-xs font-semibold text-aura-danger">Duplicate in {duplicate.listLabel}</p><p className="mb-0 mt-1 break-words text-xs text-aura-danger">“{duplicate.text}” repeats item {duplicate.firstIndex + 1}.</p>{editable ? <Button className="mt-2 h-8" variant="ghost" onClick={() => removeDuplicate(key, duplicate.duplicateIndex)}>Remove duplicate</Button> : null}</div>)}</Card>
        })}</div>
      </main>
      <aside><Card className="p-5 xl:sticky xl:top-4"><h2 className="m-0 text-base font-semibold text-depth">Analysis readiness</h2><p className="mb-0 mt-3 text-sm text-aura-text-secondary">{readiness.evidenceCount} evidence · {readiness.strongEvidenceCount} strong · {readiness.moderateEvidenceCount} moderate · {readiness.limitedEvidenceCount} limited</p>{readiness.generalBlockingIssues.map((item) => <p className="mb-0 mt-2 text-sm text-aura-danger" key={item}>{item}</p>)}{readiness.evidenceIssues.length ? <div className="mt-4 rounded-aura-sm bg-aura-danger-soft p-3"><p className="m-0 text-sm font-semibold text-aura-danger">{readiness.evidenceIssues.length} evidence {readiness.evidenceIssues.length === 1 ? 'item needs' : 'items need'} correction</p><Button className="mt-2 h-8" variant="ghost" onClick={reviewFirstIssue}>Review first issue</Button></div> : null}{readiness.duplicateListItems.length ? <div className="mt-4 rounded-aura-sm bg-aura-danger-soft p-3"><p className="m-0 text-sm font-semibold text-aura-danger">{readiness.duplicateListItems.length} duplicate {readiness.duplicateListItems.length === 1 ? 'entry needs' : 'entries need'} removal</p>{editable ? <Button className="mt-2 h-8" variant="ghost" onClick={cleanGeneratedLists}>Clean up generated analysis</Button> : null}</div> : null}{readiness.warnings.map((item) => <p className="mb-0 mt-2 text-xs text-aura-warning" key={item}>{item}</p>)}{editable ? <Button className="mt-5 w-full" disabled={!readiness.ready} onClick={() => setApproveOpen(true)}>Approve interview analysis</Button> : <><p className="mb-0 mt-4 text-sm font-semibold text-aura-success">Approved by {analysis.approvedBy}. Ready for final evaluation.</p><Link className={`${linkClass} mt-4 w-full`} to={`/candidates/${candidate.id}`}>View candidate</Link></>}</Card></aside>
    </div>
