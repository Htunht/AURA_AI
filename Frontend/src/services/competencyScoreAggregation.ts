...
export function aggregateCompetencyScores(input: { finalEvaluationId: string; interviewId: string; rubric: PublishedInterviewScoringRubric; questionAssessments: InterviewQuestionAssessment[]; evidence: InterviewEvidence[]; createdAt: string; questionPriorities?: Record<string, 'CORE' | 'FOLLOW_UP' | 'OPTIONAL'> }): CompetencyAggregationResult {
  ...
  const competencyAssessments = input.rubric.competencies.map((rule) => {
    ...
    const matchedBy = [
      ...rule.requirementIds.flatMap((id) => questions.some((item) => item.requirementIds.includes(id)) ? [{ type: 'REQUIREMENT' as const, label: rule.requirementLabels?.[id] ?? id }] : []),
      ...rule.criterionKeys.flatMap((key) => questions.some((item) => item.criterionKeys.includes(key)) ? [{ type: 'CRITERION' as const, label: key.replaceAll('_', ' ') }] : []),
      ...rule.questionIds.flatMap((id) => questions.some((item) => item.questionId === id) ? [{ type: 'QUESTION' as const, label: `Question ${id}` }] : []),
    ].filter((item, index, items) => items.findIndex((entry) => entry.type === item.type && entry.label === item.label) === index)
    ...
    return {
      ...
      systemRating, normalizedScore: systemRating === undefined ? undefined : normalized(systemRating), confidence, questionAssessmentIds: questions.map((item) => item.id), evidenceIds, requirementIds: [...rule.requirementIds], criterionKeys: [...rule.criterionKeys], mappingDiagnostic: { matchedBy, questionCount: questions.length, candidateAnswerSegmentCount: new Set(questions.flatMap((item) => item.transcriptSegmentIds)).size, evidenceItemCount: evidenceIds.length, ...(questions.length ? {} : { unresolvedReason: 'No approved questions share this competency’s requirement or criterion mappings.' }) },
