...
export function generateScreeningRules({ job, requirements, form, previousRubric }: { job: Job; requirements: JobRequirement[]; form: ApplicationForm; previousRubric?: EvaluationRubric }): GeneratedScreeningRuleSet {
  ...
  const criteria = [
    { key: 'required_qualifications', name: 'Requirements', description: `Evidence for ${requirements.filter((item) => item.importance === 'REQUIRED').map((item) => item.label).join(', ')}.`, evaluationGuidance: 'Missing required evidence lowers this score and creates a recruiter-review flag.', weight: 40 },
    { key: 'relevant_experience', name: 'Working Experience', description: `Experience aligned with ${job.title} responsibilities.`, evaluationGuidance: 'Compare submitted years and examples with the role scope.', weight: 25 },
    { key: 'role_evidence', name: 'Teamwork', description: 'Strength and specificity of mapped required and preferred qualification evidence.', evaluationGuidance: 'Reward concrete contribution and outcomes; preferred evidence is additive.', weight: 20 },
    { key: 'problem_solving', name: 'Problem-solving ability', description: 'Evidence of structured problem solving and judgment.', evaluationGuidance: 'Review the mapped problem-solving response.', weight: 10 },
    { key: 'communication', name: 'Communication skills', description: 'Clarity and completeness of submitted evidence.', evaluationGuidance: 'Use mapped supporting answers without treating style as a qualification.', weight: 5 },
  ]
